...
const PACKAGES = ["common", "math", "element", "xcalidraw"];
 ...
 * - yarn release                                 -> publishes `@xcalidraw` packages with "test" tag and "-[hash]" version suffix
 * - yarn release --tag=test                      -> same as above
 * - yarn release --tag=next                      -> publishes `@xcalidraw` packages with "next" tag and version "-[hash]" suffix
 * - yarn release --tag=next --non-interactive    -> skips interactive prompts (runs on CI/CD), otherwise same as above
 * - yarn release --tag=latest --version=0.19.0   -> publishes `@xcalidraw` packages with "latest" tag and version "0.19.0" & prepares changelog for the release
 *
...
const getArguments = () => {
  ...
  if (!version) {
    // set the next version based on the xcalidraw package version + commit hash
    const xcalidrawPackageVersion = require(getPackageJsonPath(
      "xcalidraw",
    )).version;

    const hash = getShortCommitHash();

    if (!xcalidrawPackageVersion.includes(hash)) {
      version = `${xcalidrawPackageVersion}-${hash}`;
    } else {
      // ensuring idempotency
      version = xcalidrawPackageVersion;
    }
  }
  ...
  for (const packageName of PACKAGES) {
    ...
    if (pkg.dependencies) {
      for (const dependencyName of PACKAGES) {
        if (!pkg.dependencies[`@xcalidraw/${dependencyName}`]) {
          continue;
        }

        pkg.dependencies[`@xcalidraw/${dependencyName}`] = nextVersion;
      }
    }
    ...
    rl.question(
      ...
      (answer) => {
        ...
        if (answer.toLowerCase() === "y") {
          execSync(`git add -u`);
          execSync(
            `git commit -m "chore: release @xcalidraw/xcalidraw@${nextVersion} 🎉"`,
          );
        } else {
          console.warn(
...
const buildPackages = () => {
  ...
  for (const packageName of PACKAGES) {
    console.info(`Building "@xcalidraw/${packageName}"...`);
...
const publishPackages = (tag, version) => {
  for (const packageName of PACKAGES) {
    ...
    console.info(
      `Published "@xcalidraw/${packageName}@${tag}" with version "${version}"! 🎉`,
    );
  }
};
