...
Xcalidraw uses CSS variables to style certain components. To override them, you should set your own on the `.xcalidraw` and `.xcalidraw.theme--dark` (for dark mode variables) selectors.
...
For a complete list of variables, check [theme.scss](https://github.com/xcalidraw/xcalidraw/blob/master/packages/xcalidraw/css/theme.scss), though most of them will not make sense to override.

```css showLineNumbers
.custom-styles .xcalidraw {
...
.custom-styles .xcalidraw.theme--dark {
