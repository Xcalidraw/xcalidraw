import { pointFrom, type LocalPoint } from "@xcalidraw/math";
...
import { bindLinearElement } from "@xcalidraw/element";
import {
  newArrowElement,
  newElement,
  newFrameElement,
...
import { measureText, normalizeText } from "@xcalidraw/element";
import { isArrowElement } from "@xcalidraw/element";

import { syncInvalidIndices } from "@xcalidraw/element";

import { redrawTextBoundingBox } from "@xcalidraw/element";

import { LinearElementEditor } from "@xcalidraw/element";

import { getCommonBounds } from "@xcalidraw/element";

import { Scene } from "@xcalidraw/element";

import type { ElementConstructorOpts } from "@xcalidraw/element";
...
import type { MarkOptional } from "@xcalidraw/common/utility-types";
  ...
  label?: {
    ...
    | (
        ...
        | ((
            ...
            | {
                type?: "text";
                id: XcalidrawTextElement["id"];
                text: string;
              }
          ) &
            Partial<XcalidrawTextElement>)
      ) &
        MarkOptional<ElementConstructorOpts, "x" | "y">;
  start?:
    | (
        ...
        MarkOptional<ElementConstructorOpts, "x" | "y">;
} & Partial<XcalidrawLinearElement>;

export type ValidContainer =
  | {
      type: Exclude<XcalidrawGenericElement["type"], "selection">;
      id?: XcalidrawGenericElement["id"];
      label?: {
        text: string;
        fontSize?: number;
...
export type XcalidrawElementSkeleton =
  | Extract<
      Exclude<XcalidrawElement, XcalidrawSelectionElement>,
      XcalidrawIframeLikeElement | XcalidrawFreeDrawElement
    >
  | ({
      type: Extract<XcalidrawLinearElement["type"], "line">;
      x: number;
      y: number;
    } & Partial<XcalidrawLinearElement>)
  | ValidContainer
  ...
  | ({
      type: Extract<XcalidrawImageElement["type"], "image">;
      x: number;
      y: number;
      fileId: FileId;
    } & Partial<XcalidrawImageElement>)
  | ({
      type: "frame";
      children: readonly XcalidrawElement["id"][];
      name?: string;
    } & Partial<XcalidrawFrameElement>)
  | ({
      type: "magicframe";
      children: readonly XcalidrawElement["id"][];
      name?: string;
    } & Partial<XcalidrawMagicFrameElement>);
...
const bindTextToContainer = (
  ...
  const textElement: XcalidrawTextElement = newTextElement({
    x: 0,
    y: 0,
...
const bindLinearElementToElement = (
  linearElement: XcalidrawArrowElement,
  start: ValidLinearElement["start"],
  end: ValidLinearElement["end"],
  elementStore: ElementStore,
  scene: Scene,
): {
  linearElement: XcalidrawLinearElement;
  startBoundElement?: XcalidrawElement;
  endBoundElement?: XcalidrawElement;
} => {
  ...
  if (start) {
    ...
    if (startType) {
      ...
      bindLinearElement(
        linearElement,
        startBoundElement as XcalidrawBindableElement,
        "start",
        scene,
      );
    }
  ...
  if (end) {
    ...
    if (endType) {
      ...
      bindLinearElement(
        linearElement,
        endBoundElement as XcalidrawBindableElement,
        "end",
        scene,
      );
    }
...
class ElementStore {
  xcalidrawElements = new Map<string, XcalidrawElement>();

  add = (ele?: XcalidrawElement) => {
    if (!ele) {
      return;
    }

    this.xcalidrawElements.set(ele.id, ele);
  };

  getElements = () => {
    return syncInvalidIndices(Array.from(this.xcalidrawElements.values()));
  };
  ...
  getElement = (id: string) => {
    return this.xcalidrawElements.get(id);
  };
}

export const convertToXcalidrawElements = (
  elementsSkeleton: XcalidrawElementSkeleton[] | null,
  ...
  const elementsWithIds = new Map<string, XcalidrawElementSkeleton>();
  const oldToNewElementIdMap = new Map<string, string>();

  // Create individual elements
  for (const element of elements) {
    let xcalidrawElement: XcalidrawElement;
    ...
    switch (element.type) {
      ...
      case "line": {
        ...
        xcalidrawElement = newLinearElement({
          width,
          height,
      ...
      case "arrow": {
        const width = element.width || DEFAULT_LINEAR_ELEMENT_PROPS.width;
        const height = element.height || DEFAULT_LINEAR_ELEMENT_PROPS.height;
        xcalidrawElement = newArrowElement({
          width,
          height,
          endArrowhead: "arrow",
          points: [pointFrom(0, 0), pointFrom(width, height)],
        ...
        Object.assign(
          xcalidrawElement,
          getSizeFromPoints(xcalidrawElement.points),
        );
        break;
      }
      case "text": {
        ...
        xcalidrawElement = newTextElement({
          width: metrics.width,
      ...
      case "image": {
        xcalidrawElement = newImageElement({
      ...
      case "frame": {
        xcalidrawElement = newFrameElement({
          x: 0,
          y: 0,
          ...element,
        });
      ...
      case "magicframe": {
        xcalidrawElement = newMagicFrameElement({
          x: 0,
          y: 0,
          ...element,
        });
    ...
    const existingElement = elementStore.getElement(xcalidrawElement.id);
    if (existingElement) {
      console.error(`Duplicate id found for ${xcalidrawElement.id}`);
    } else {
      elementStore.add(xcalidrawElement);
      elementsWithIds.set(xcalidrawElement.id, element);
      if (originalId) {
        oldToNewElementIdMap.set(originalId, xcalidrawElement.id);
      }
    }
  }
  ...
  for (const [id, element] of elementsWithIds) {
    const xcalidrawElement = elementStore.getElement(id)!;
        ...
        if (element.label?.text) {
          ...
          switch (element.type) {
            case "arrow": {
              ...
              const { linearElement, startBoundElement, endBoundElement } =
                bindLinearElementToElement(
                  xcalidrawElement as XcalidrawArrowElement,
                  start,
                  end,
                  elementStore,
                  scene,
  ...
  // Once all the xcalidraw elements are created, we can add frames since we
  ...
  for (const [id, element] of elementsWithIds) {
    ...
    if (!frame) {
      throw new Error(`Xcalidraw element with id ${id} doesn't exist`);
    }
    const childrenElements: XcalidrawElement[] = [];
