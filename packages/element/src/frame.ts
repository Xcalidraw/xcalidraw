import { arrayToMap } from "@xcalidraw/common";
import { isPointWithinBounds, pointFrom } from "@xcalidraw/math";
import { doLineSegmentsIntersect } from "@xcalidraw/utils/bbox";
import { elementsOverlappingBBox } from "@xcalidraw/utils/withinBounds";
...
import type { ReadonlySetLike } from "@xcalidraw/common/utility-types";
...
import type { XcalidrawElementsIncludingDeleted } from "./Scene";
...
export const bindElementsToFramesAfterDuplication = (
  ...
  origElements: readonly XcalidrawElement[],
 ...
 */
export const groupByFrameLikes = (elements: readonly XcalidrawElement[]) => {
...
export const getFrameChildren = (
  ...
  const frameChildren: XcalidrawElement[] = [];
...
export const getFrameLikeElements = (
  allElements: XcalidrawElementsIncludingDeleted,
): XcalidrawFrameLikeElement[] => {
  return allElements.filter((element): element is XcalidrawFrameLikeElement =>
    isFrameLikeElement(element),
  );
};

/**
 * Returns XcalidrawFrameElements and non-frame-children elements.
 *
...
export const getElementsInResizingFrame = (
  ...
  const nextElementsInFrame = new Set<XcalidrawElement>(prevElementsInFrame);
...
export const getElementsInNewFrame = (
  elements: XcalidrawElementsIncludingDeleted,
  frame: XcalidrawFrameLikeElement,
  elementsMap: ElementsMap,
) => {
...
export const getContainingFrame = (
  ...
  return (elementsMap.get(element.frameId) ||
    null) as null | XcalidrawFrameLikeElement;
};
...
export const filterElementsEligibleAsFrameChildren = (
  ...
  const otherFrames = new Set<XcalidrawFrameLikeElement["id"]>();
  const elementsMap = arrayToMap(elements);
  ...
  const processedGroups = new Set<XcalidrawElement["id"]>();

  const eligibleElements: XcalidrawElement[] = [];
...
export const addElementsToFrame = <T extends ElementsMapOrArray>(
  allElements: T,
  elementsToAdd: NonDeletedXcalidrawElement[],
  frame: XcalidrawFrameLikeElement,
  appState: AppState,
): T => {
  const elementsMap = arrayToMap(allElements);
  const currTargetFrameChildrenMap = new Map<XcalidrawElement["id"], true>();
  for (const element of allElements.values()) {
  ...
  const finalElementsToAdd: XcalidrawElement[] = [];
...
export const removeElementsFromFrame = (
  elementsToRemove: ReadonlySetLike<NonDeletedXcalidrawElement>,
  elementsMap: ElementsMap,
) => {
...
export const removeAllElementsFromFrame = <T extends XcalidrawElement>(
...
export const replaceAllElementsInFrame = <T extends XcalidrawElement>(
  allElements: readonly T[],
  nextElementsInFrame: XcalidrawElement[],
  frame: XcalidrawFrameLikeElement,
...
>(
  ...
  const elementsToFilter = new Set<XcalidrawElement>(selectedElements);
  ...
  const elementsToRemove = new Set<XcalidrawElement>();
...
export const isElementInFrame = (
  ...
  appState: StaticCanvasAppState,
  opts?: {
    targetFrame?: XcalidrawFrameLikeElement;
...
export const getDefaultFrameName = (element: XcalidrawFrameLikeElement) => {
  // TODO name frames "AI" only if specific to AI frames
  return isFrameElement(element) ? DEFAULT_FRAME_NAME : DEFAULT_AI_FRAME_NAME;
};

export const getFrameLikeTitle = (element: XcalidrawFrameLikeElement) => {
